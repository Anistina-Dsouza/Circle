...
import { TrendingUp, Layout, Activity } from 'lucide-react';
...
const GrowthTrends = ({ trends, hideDistribution = false }) => {
    ...
    return (
        <div className={`grid ${hideDistribution ? 'grid-cols-1' : 'lg:grid-cols-2'} gap-6 sm:gap-10`}>
            {/* Registration Trends - GROWTH VELOCITY */}
            <div className="rounded-[24px] sm:rounded-[32px] bg-[#1A0C3F]/50 backdrop-blur-xl border border-white/10 shadow-2xl transition-all duration-500 hover:border-purple-500/20 relative">
                {/* Standardized Header */}
                ...
                <div className="p-6 sm:p-10">
                    <div className="relative h-[240px] sm:h-[280px]">
                        ...
                        <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pr-4 sm:pr-8">
                            {[100, 50, 0].map((percent) => (
                                <div key={percent} className="flex items-center gap-4 sm:gap-8 w-full group/grid">
                                    <span className="text-[10px] sm:text-[12px] font-black text-white/60 w-8 sm:w-10 text-right tabular-nums">
                        ...
                        <div className="absolute inset-0 left-12 sm:left-16 px-2 sm:px-6 flex items-end justify-between gap-2 sm:gap-4">
                            {registrations.length > 0 ? registrations.map((d, i) => (
                                <div key={i} className="flex-1 flex flex-col items-center group/bar h-full">
                                    <div className="relative w-full h-full flex items-end justify-center">
                                        ...
                                        <div className="absolute inset-x-0 bottom-0 top-0 w-full max-w-[20px] sm:max-w-[24px] mx-auto bg-purple-500/[0.02] rounded-full border border-white/[0.03] pointer-events-none" />
 
                                        {/* The Pillar - Segmented Design */}
                                        <div 
                                            className="w-full max-w-[20px] sm:max-w-[24px] rounded-full transition-all duration-1000 ease-out relative z-10 overflow-hidden
                                                bg-purple-500/10 group-hover/bar:bg-purple-500/20 group-hover/bar:shadow-[0_0_20px_rgba(168,85,247,0.2)]"
                                            style={{ height: `${(d.count / maxReg) * 100}%`, minHeight: d.count > 0 ? '8px' : '4px' }}
                                        >
                                        ...
                                        <div 
                                            className="absolute w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full transition-all duration-500 z-20 mb-[-4px] bg-white/20 border border-purple-400/30 group-hover/bar:bg-fuchsia-400 group-hover/bar:scale-150 group-hover/bar:shadow-[0_0_15px_#e879f9]"
                    ...
                    {/* Bottom X-Axis */}
                    <div className="flex mt-8 ml-12 sm:ml-16 px-2 sm:px-6 justify-between border-t border-white/5 pt-8 overflow-x-hidden">
                        {registrations.map((item, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-3 group/x-label">
                                ...
                                <span className="text-[8px] sm:text-[10px] font-black text-white/30 group-hover/x-label:text-white transition-colors uppercase tracking-tight">
                                    {formatDate(item._id)}
                                </span>
                            </div>
                        ))}
            ...
            {!hideDistribution && (
            <div className="rounded-[24px] sm:rounded-[32px] bg-[#1A0C3F]/50 backdrop-blur-xl border border-white/10 shadow-2xl transition-all duration-500 hover:border-indigo-500/20 relative">
                {/* Standardized Header */}
                <div className="flex justify-between items-center px-6 sm:px-8 py-5 sm:py-7 border-b border-white/5">
                    <div className="flex items-center gap-4 sm:gap-5">
                        <div className="p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400">
                ...
                <div className="p-6 sm:p-10 space-y-7 sm:space-y-9">
                    {categories.length > 0 ? categories.map((cat, idx) => (
                        <div key={idx} className="space-y-3 sm:space-y-4 group/row">
                            <div className="flex justify-between items-end px-1">
                                <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
                                    <span className="text-xs sm:text-sm font-bold text-white/80 group-hover/row:text-white transition-colors">{cat._id}</span>
                                </div>
