    ...
    <div className={`${isDetailed ? '' : 'mt-12 rounded-[32px] bg-[#1A0C3F]/50 backdrop-blur-xl border border-white/10 shadow-2xl'} relative transition-all duration-500 hover:border-purple-500/20`}>
      ...
      <div className="flex justify-between items-center px-8 py-7 border-b border-white/5">
        <div className="flex items-center gap-5">
           ...
           <div>
             <h2 className="text-xl font-bold text-white tracking-tight">System Resonance</h2>
             <p className="text-xs text-white/40 mt-1 uppercase tracking-widest font-black">Live Pulse Audit</p>
           </div>
        </div>

        <div className="flex items-center gap-8">
            {/* Live Counter Badge */}
            <div className="hidden lg:flex items-center gap-4 px-6 py-2.5 rounded-2xl bg-[#0F0529]/60 border border-white/5 shadow-2xl">
                <div className="relative">
                    <div className="absolute inset-0 bg-purple-500/20 rounded-full blur-md" />
                    <div className="relative w-2.5 h-2.5 rounded-full bg-purple-500 animate-pulse" />
                </div>
                <div className="flex flex-col">
                    <span className="text-[16px] font-black text-white tabular-nums leading-none">
                        {latestNodeCount}
                    </span>
                    <span className="text-[8px] font-black text-purple-400/40 uppercase tracking-widest mt-1">
                        Active Nodes
                    </span>
                </div>
            </div>
            ...
            {["Live View", "7 Days"].map((t) => (
                ...
                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-500 ${timeline === t ? "bg-purple-600 text-white shadow-xl" : "text-white/30 hover:text-white"}`}>
                {t}
                </button>
            ))}
            </div>
      ...
      <div className={`${isDetailed ? 'pt-6' : 'pt-24'} pb-10 px-10`}>
        <div className="relative h-[340px]">
            ...
            {[100, 50, 0].map((percent) => (
                ...
                <span className="text-[12px] font-black text-white/20 w-10 text-right tabular-nums">
            ...
            {/* Chart Content Area */}
            <div className="absolute inset-0 left-16 px-6">
            ...
            {timeline === "Live View" ? (
                ...
                {/* Precise Micro-Nodes (Using DIVs to prevent ellipse distortion) */}
                {chartData.map((d, i) => (
                    <div 
                        key={i}
                ...
                {active && activeData && (
                    <div 
                    className="absolute bg-[#0F0529]/90 backdrop-blur-xl text-white px-5 py-3.5 rounded-2xl text-[10px] font-black pointer-events-none z-30 transition-all duration-500 shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex flex-col items-center gap-1.5 border border-purple-500/30"
                    style={{ 
                    ...
                    >
                        ...
                        <span className="uppercase tracking-widest text-[13px] text-white tabular-nums">{activeData.value} Active Nodes</span>
                        <div className="text-[9px] text-purple-400/60 uppercase tracking-[0.2em] border-t border-white/5 pt-1.5 w-full text-center font-black mt-1.5">
                            Temporal Pulse: {active}
                        </div>
                        <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-[#0F0529]/90 border-r border-b border-purple-500/30 rotate-45" />
                    </div>
                )}
                </div>
            ) : (
                ...
                {chartData.map((item, i) => (
                    ...
                    <div className="relative w-full h-full flex items-end justify-center group/bar">
                        ...
                        <div className={`absolute bg-[#0F0529]/95 backdrop-blur-2xl text-white px-4 py-3 rounded-2xl text-[10px] font-black pointer-events-none z-30 mb-10 bottom-full transition-all duration-500 flex flex-col items-center gap-1 shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-purple-500/40 ${active === item.day ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'}`}>
                            <div className="flex items-center gap-2 whitespace-nowrap">
                                <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]" />
                                <span className="uppercase tracking-wider text-[12px]">{item.value} Active Nodes</span>
                            </div>
                            <span className="text-[8px] text-purple-400/50 uppercase tracking-[0.3em] border-t border-white/5 pt-1 w-full text-center font-black">
                                {item.day}
                            </span>
                        ...
                        `}
                        style={{ height: `${(item.value / maxVal) * 100}%`, minHeight: item.value > 0 ? '8px' : '0px' }}
                        />
                    </div>
                    </div>
                ))}
        ...
        <div className="flex mt-8 ml-16 px-6 justify-between relative border-t border-white/10 pt-10">
            ...
            <div key={i} className="flex-1 flex flex-col items-center gap-4">
                ...
                <>
                    ...
                    <span
                    className={`text-[12px] font-black tracking-widest uppercase transition-all duration-500
