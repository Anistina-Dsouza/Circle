        ...
        <div className="rounded-[24px] sm:rounded-[32px] bg-[#1A0C3F]/50 backdrop-blur-xl border border-white/10 overflow-hidden shadow-2xl transition-all duration-500">
          <div className="overflow-x-auto custom-scrollbar-horizontal">
            <table className="w-full min-w-[1000px] border-collapse">
              {/* Table Head */}
              <thead>
                <tr className="text-[10px] uppercase tracking-[0.2em] font-black text-white/30 border-b border-white/5 bg-white/[0.02]">
                  <th className="px-8 py-6 text-left w-[25%] font-black">User Profile</th>
                  <th className="px-6 py-6 text-left w-[20%] font-black hidden md:table-cell">Email Access</th>
                  <th className="px-4 py-6 text-center w-[12%] font-black">Node Role</th>
                  <th className="px-4 py-6 text-center w-[13%] font-black">Telemetry Flags</th>
                  <th className="px-6 py-6 text-center w-[15%] font-black hidden lg:table-cell">Join Date</th>
                  <th className="px-8 py-6 text-right w-[15%] font-black">Matrix Control</th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-white/5">
                {loading ? (
                  ...
                  return processed.map(user => (
                    <tr 
                      key={user._id} 
                      className={`group transition-all duration-300 ${user.isActive ? 'hover:bg-white/[0.03]' : 'bg-red-900/5 opacity-75 grayscale-[20%]'}`}
                    >
                      {/* User */}
                      ...
                      <td className="px-6 py-6 hidden md:table-cell">
                        <div className="text-white/60 text-sm font-medium truncate max-w-[180px] tabular-nums">
                          {user.email}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-6 text-center">
                        <div className="flex justify-center">
                          {user.pendingReports > 0 ? (
                            ...
                            >
                              <ShieldAlert size={12} />
                              <span className="whitespace-nowrap">{user.pendingReports} PENDING</span>
                            </button>
                          ) : (
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end">
                          {user.role === 'admin' ? (
                            <div className="flex items-center gap-2 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest bg-white/5 text-white/20 border border-white/5 opacity-50 whitespace-nowrap">
                              <span>PROTECTED NODE</span>
                            </div>
                          ) : (
                            <button 
                              onClick={() => toggleStatus(user._id, user.role)}
                              className={`flex items-center gap-2 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg whitespace-nowrap ${
                                user.isActive 
